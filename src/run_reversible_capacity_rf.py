"""Bayesian-optimized RF for reversible capacity using the original six features."""
...
def main():
    ...
    metrics_path = outdir / 'rf_metrics_reversible.json'
    ...
    ]).to_csv(outdir / 'rf_metrics_summary_reversible.csv', index=False)

    pd.DataFrame({'y_true': y_train, 'y_pred': y_pred_train_cv}).to_csv(outdir / 'rf_predictions_train_cv_reversible.csv', index=False)
    pd.DataFrame({'y_true': y_test, 'y_pred': y_pred_test}).to_csv(outdir / 'rf_predictions_test_reversible.csv', index=False)
    fold_metrics_frame(cv_results).to_csv(outdir / 'rf_cv_fold_metrics_reversible.csv', index=False)
    dump(best_model, outdir / 'rf_model_reversible.joblib')
